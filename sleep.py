...
if __name__ == "__main__":
    while True:
        hddidle = check_drive_status()
        cpuidle = check_cpu_status()
        netidle = check_network_status()
        hostsoffline = pings()
        solar = getSolar()

        print(hddidle, cpuidle, netidle, hostsoffline, solar)
        if hddidle == cpuidle == netidle == hostsoffline == solar == 1:
            hibernate()
            break
        else:
